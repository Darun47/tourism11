    ...
    def _generate_mock_response(
        ...
        if any(word in message_lower for word in ['hello', 'hi', 'hey']):
            return {
                ...
                'message': (
                    "Hello! 👋 I'm your AI travel assistant. I can help you plan a cultural trip, "
                    "suggest destinations, and build itineraries.\n\n"
                    "To get started, tell me:\n"
                    "• Your interests (art, history, nature, food, etc.)\n"
                    "• Budget range (budget/mid-range/luxury)\n"
                    "• Preferred season or climate (summer, winter, temperate)\n"
                    "• Trip length\n"
                ),
                'type': 'greeting',
                'suggestions': [
        ...
        # Season-specific recommendations
        elif any(season in message_lower for season in ['spring', 'summer', 'autumn', 'fall', 'winter']):
            season_map = {
                'spring': 'Spring',
                'summer': 'Summer',
                'autumn': 'Autumn',
            ...
            matched_season = next(
                (season_map[key] for key in season_map if key in message_lower),
                None
            )
            if matched_season:
                seasonal = self.engine.get_seasonal_recommendations(
                    season=matched_season,
                    budget=None,
                    num_recommendations=3
                )
                destinations = [
                    f"{rec['city']} ({rec['country']})"
                    for rec in seasonal.get('recommendations', [])
                ]
                destination_text = ", ".join(destinations) if destinations else "several great options"
                return {
                    'message': (
                        f"Here are top {matched_season} picks: {destination_text}. "
                        "Would you like a full itinerary or budget-friendly options for any of these?"
                    ),
        ...
        # Default response
            ...
            return {
                ...
                'message': (
                    ...
                    "• Accessibility options\n"
                    "• Cultural insights\n\n"
                    "Tell me your interests, budget, and preferred season or climate to get tailored advice."
                ),
                'type': 'help',
                'suggestions': [
